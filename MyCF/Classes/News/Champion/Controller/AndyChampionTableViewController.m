//
//  AndyChampionTableViewController.m
//  MyCF
//
...
@interface AndyChampionTableViewController ()

@end

@implementation AndyChampionTableViewController

- (void)viewDidLoad
{
    [super viewDidLoad];
    
    AndyLog(@"ChampionTableViewController 被加载");
}
...
- (void)loadNewData
{
    ...
    [AndyHttpTool getWithURL:[NSString stringWithFormat:@"http://qt.qq.com/php_cgi/cf_news/php/varcache_getnews.php?id=23&page=%d", self.pageIndex] params:nil success:^(id json) {
        ...
        // 缓存
        [[NSString stringWithFormat:@"%@", json] saveContentToFile:ChampionCacheFileName atomically:YES];
        
    } failure:^(NSError *error)
     {
         NSString *cacheFilePath = [AndyCommonFunction getCacheFilePathWithFileName:ChampionCacheFileName];
