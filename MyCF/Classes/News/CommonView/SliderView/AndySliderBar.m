...
@interface AndySliderBar () <AndysliderViewDelegate>

@property (nonatomic, strong) NSMutableArray *slideBarButtons;

@property (nonatomic, weak) AndySliderBarButton *selectedButton;
...
- (void)setupSliderBarButtonsArray:(NSArray *)titleArr
{
    ...
    for (int i = 0; i < titleArr.count; i++)
    {
        ...
        AndySliderBarButton *slideBarButton = [[AndySliderBarButton alloc] init];
...
- (void)buttonClick:(AndySliderBarButton *)button
{
    if ([self.delegate respondsToSelector:@selector(sliderBar:didSelectedButtonFrom:to:)])
    {
        [self.delegate sliderBar:self didSelectedButtonFrom:self.selectedButton.tag to:button.tag];
    }
...
- (void)layoutSubviews
{
    ...
    for (int index = 0; index < self.slideBarButtons.count; index++)
    {
        ...
        AndySliderBarButton *slideBarButton = self.slideBarButtons[index];
    ...
    CGFloat bottomLineViewX = self.slideBarButtons.count == 3 ? 35 : 55;
    CGFloat bottomLineViewY = self.frame.size.height - 5;
    CGFloat bottomLineViewW = slideBarButtonW - 2 * (self.slideBarButtons.count == 3 ? 35 : 55);
...
- (void)sliderView:(AndySliderView *)slideView didScrollViewTo:(int)to
{
    self.selectedButton.selected = NO;
    
    ((AndySliderBarButton *)self.slideBarButtons[to]).selected = YES;
...
- (void)animateBottomLine:(int)to
{
    [UIView animateWithDuration:0.2 animations:^{
        self.bottomLineView.frame = (CGRect){{to * self.itemWith + (self.slideBarButtons.count == 3 ? 35 : 55), self.bottomLineView.frame.origin.y}, self.bottomLineView.frame.size};
    }];
}
